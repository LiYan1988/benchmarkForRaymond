...
for i=1:nSimulations
    ...
    fid = fopen(sprintf('%s/dmd%d_%d_%s.m', folderName, nDemands, i, modulation), 'w');
    fprintf(fid, '%s\n', template_matlab{:});
    fclose(fid);
end
...
template_slurm{5} = sprintf('#SBATCH --output=%ddmd_%%a.stdout', nDemands);
template_slurm{6} = sprintf('#SBATCH --error=%ddmd_%%a.stderr', nDemands);
...
template_slurm{16} = sprintf('matlab -nodesktop -r "dmd%d_${SLURM_ARRAY_TASK_ID}_%s;" -logfile matlab_output_${SLURM_ARRAY_TASK_ID}',nDemands, modulation);
